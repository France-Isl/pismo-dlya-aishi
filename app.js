(() => {
  ...
  function applyLanguage(render = true) {
    ...
    setText("#rainToggle b", rainScene.enabled ? t("stateOn") : t("stateOff")); setText("#natureToggle b", isNaturePlaying ? t("stateOn") : t("stateOff")); if (!$("#weatherState").textContent.includes("°")) setText("#weatherState", weatherEnabled ? t("stateOn") : t("stateOff")); setText("#fullscreenToggle b", t("stateOpen"));
  ...
  function bindEvents(){
    ...
    $("#settingsButton").addEventListener("click",()=>{pendingPremiumFeature="";openPanel(layers.settings);});$("#settingsClose").addEventListener("click",()=>closePanel(layers.settings));$("#settingsBackdrop").addEventListener("click",()=>closePanel(layers.settings));
  ...
  async function init(){
    ...
    if("serviceWorker" in navigator&&location.protocol.startsWith("http")&&location.hostname!=="appassets.androidplatform.net"){
      const registerServiceWorker=()=>navigator.serviceWorker.register("sw.js?v=6").catch(()=>{});
